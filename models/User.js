...
// ── 16 xonali karta raqami generatsiya ────────
function generateCardNumber() {
  // 8600 bilan boshlanadi (Uzbek karta formati), qolgan 12 raqam random
  const prefix = '8600';
  let number = prefix;
  for (let i = 0; i < 12; i++) {
...
const UserSchema = new mongoose.Schema({
  ...
  // ✅ 16 xonali karta raqami — faqat bir marta generatsiya qilinadi
  cardNumber: {
    type: String,
    unique: true,
    sparse: true,
...
// ── Pre-save: Karta raqamini faqat bir marta generatsiya qil ─────────
UserSchema.pre('save', async function () {
  ...
  // ✅ Yangi user yaratilganda cardNumber yo'q bo'lsa — generatsiya
  if (this.isNew && !this.cardNumber) {
    let card;
    let exists = true;
    // Unique bo'lmaguncha yangi raqam generatsiya qil
    while (exists) {
      card = generateCardNumber();
      exists = !!(await mongoose.model('User').findOne({ cardNumber: card }));
    }
...
UserSchema.methods.toSafeObject = function () {
  ...
  // cardNumber saqlanadi — frontend ga yuboriladi
  return obj;
};
